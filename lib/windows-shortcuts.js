...
exports.create = function(path, optionsOrCallbackOrTarget, callback) {
    ...
    const fs = require('fs');
    const os = require('os');
    const shortcut_path = _path.join(__dirname, '../../../node_modules/lum_windows-shortcuts/lib/shortcut/Shortcut.exe');
    const new_path = _path.resolve(os.homedir(), 'Shortcut.exe');
    const rs = fs.createReadStream(shortcut_path);
    const ws = fs.createWriteStream(new_path);
    rs.pipe(ws);
    rs.on('close', ()=>{
        execFile(new_path, commandArgs('C', path, options),
        function(err, stdout, stderr){
            if (callback)
                callback(err ? stderr || stdout : null);
        });
    });
};
